...
// Package-level random source for better performance
var (
	rng     *rand.Rand
	rngOnce sync.Once
	rngMu   sync.Mutex
...
// getRng returns the package-level random source (lazy initialization)
...
func calculateBackoff(attempt int, config RetryConfig) time.Duration {
	...
	// Get random backoff between min and max (thread-safe)
	rngMu.Lock()
	randomFactor := getRng().Float64()
	rngMu.Unlock()
	backoff = min + (max-min)*randomFactor
