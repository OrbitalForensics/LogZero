...
func (p *LogParser) Parse(filePath string) ([]*core.Event, error) {
	...
	// Pre-allocate slice with estimated capacity (avg 100 bytes per log line)
	events := make([]*core.Event, 0, estimateLineCapacity(filePath, 100))
	lineNum := 0
	...
	for scanner.Scan() {
		...
		// Truncate line for regex matching to prevent ReDoS on extremely long lines
		lineForRegex := truncateLine(line)

		// If we haven't detected the format yet, try all patterns
		if detectedPatternIndex == -1 {
			timestamp, timeStr, detectedPatternIndex = extractTimestampWithDetection(lineForRegex)
		} else {
			// Use the detected pattern
			timestamp, timeStr = extractTimestampWithPattern(lineForRegex, detectedPatternIndex)
		}
