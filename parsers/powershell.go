...
func (p *PowerShellTranscriptParser) CanParse(filePath string) bool {
	...
	// Check file content for PowerShell transcript markers using cached header
	lines, err := getFileHeader(filePath)
	if err != nil {
		return false
	}

	// Check first 10 lines for transcript markers
	...
	for i := 0; i < checkLines; i++ {
		...
		if transcriptStartPattern.MatchString(lines[i]) || transcriptHeaderMarker.MatchString(lines[i]) {
			return true
		}
	}
...
func (p *PowerShellTranscriptParser) Parse(filePath string) ([]*core.Event, error) {
	...
	for scanner.Scan() {
		...
		if transcriptStartPattern.MatchString(lineForRegex) {
			if !headerParsed {
				inHeader = true
			}
			continue
		...
		if inHeader && !headerParsed {
			if transcriptHeaderMarker.MatchString(lineForRegex) {
				continue
			}
			if transcriptEndMarker.MatchString(lineForRegex) {
			...
			// Extract start time
			if matches := transcriptStartTime.FindStringSubmatch(lineForRegex); matches != nil {
				startTime = parseTranscriptTimestamp(matches[1])
				continue
			}

			// Extract end time (might appear in footer)
			if matches := transcriptEndTime.FindStringSubmatch(lineForRegex); matches != nil {
				endTime = parseTranscriptTimestamp(matches[1])
				continue
			}

			// Extract username
			if matches := transcriptUsername.FindStringSubmatch(lineForRegex); matches != nil {
				username = strings.TrimSpace(matches[1])
				continue
			}

			// Extract RunAs user
			if matches := transcriptRunAsUser.FindStringSubmatch(lineForRegex); matches != nil {
				runAsUser = strings.TrimSpace(matches[1])
				continue
			}

			// Extract machine name
			if matches := transcriptMachine.FindStringSubmatch(lineForRegex); matches != nil {
				machine = strings.TrimSpace(matches[1])
				continue
			}

			// Extract host application
			if matches := transcriptHostApp.FindStringSubmatch(lineForRegex); matches != nil {
		...
		if matches := transcriptPromptPattern.FindStringSubmatch(lineForRegex); matches != nil {
...
func (p *PowerShellScriptBlockParser) CanParse(filePath string) bool {
	...
	// Check file content for Script Block markers using cached header
	lines, err := getFileHeader(filePath)
	if err != nil {
		return false
	}

	// Check lines for script block markers (cache already reads up to 50 lines)
...
func (p *PowerShellScriptBlockParser) Parse(filePath string) ([]*core.Event, error) {
	...
	for scanner.Scan() {
		...
		if matches := scriptBlockTimestamp.FindStringSubmatch(lineForRegex); matches != nil {
			ts, err := time.Parse(time.RFC3339, matches[1])
			if err == nil {
				currentTimestamp = ts.UTC()
			}
		} else if matches := scriptBlockTimestamp2.FindStringSubmatch(lineForRegex); matches != nil {
		...
		if matches := scriptBlockMessageNumber.FindStringSubmatch(lineForRegex); matches != nil {
			fmt.Sscanf(matches[1], "%d", &currentMessageNumber)
		}

		// Extract message total
		if matches := scriptBlockMessageTotal.FindStringSubmatch(lineForRegex); matches != nil {
			fmt.Sscanf(matches[1], "%d", &currentMessageTotal)
		}

		// Extract script path if present
		if matches := scriptBlockPath.FindStringSubmatch(lineForRegex); matches != nil {
			currentPath = matches[1]
		}

		// Check for XML-style ScriptBlockText
		if matches := scriptBlockTextPattern.FindStringSubmatch(lineForRegex); matches != nil {
